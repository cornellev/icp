...
typedef std::vector<icp::Vector, Eigen::aligned_allocator<icp::Vector>> AlignedVectorOfVectors;
...
// }
AlignedVectorOfVectors parse_ply(const std::string& path) {
    pcl::PointCloud<pcl::PointXYZ>::Ptr cloud(new pcl::PointCloud<pcl::PointXYZ>);
    if (pcl::io::loadPLYFile<pcl::PointXYZ>(path, *cloud) == -1) {
        throw std::runtime_error("failed to read PLY file: failed to open file");
    }

    AlignedVectorOfVectors points;
...
void write_transformed_points_ply(const AlignedVectorOfVectors& points,
    const icp::RBTransform& transform, const std::string& output_path) {
    pcl::PointCloud<pcl::PointXYZ>::Ptr cloud(new pcl::PointCloud<pcl::PointXYZ>);
...
void test_icp_ply(const icp::ICP::Config& config, const std::string& path_a,
    ...
    AlignedVectorOfVectors a = parse_ply(path_a);
    AlignedVectorOfVectors b = parse_ply(path_b);

    std::vector<icp::Vector> a_std(a.begin(), a.end());
    std::vector<icp::Vector> b_std(b.begin(), b.end());
    auto result = driver.converge(a_std, b_std, icp::RBTransform(3));
    write_transformed_points_ply(a, result.transform, output_path);
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        test_icp_ply(config, path_a, path_b, output_path);
